...
--TASK 5
SELECT roomno,hotelno FROM booking WHERE year(datefrom) BETWEEN 2005 AND 2010  

--TASK 6
SELECT * FROM room WHERE hotelno = 'ch01' AND type = 'single' AND price BETWEEN 20 AND 40

--Task7
select roomno,hotelno from room where price = (select  max(price) from room)

--TASK8(a)
select CONCAT('The Hotel ID is ',hotelno,' is in ',city,' and its Name is ',hotelname) from hotel

--Task8(b)
select 'The Hotel ID is '+hotelno+' is in '+city+' and its Name is '+hotelname from hotel

--Task9
select * from hotel where hotelno in
(select hotelno from room where type='double' or type='family')

--Task10
select * from hotel where hotelno in
(select hotelno from room where type !='family' and (type='double' or type='family'))

--Task11
select * from room where roomno in
(select roomno from booking where guestno!=1001 and (guestno=10003 or guestno=1007))
