...
select cardNum from [Card] where balance = (select max(balance) from [Card])))

--Task3
select u.name,(select name from usertype where  u.userId=userTypeId)
 as typeName from [user] u

--Task4
select userID from [userCard] where cardnum in(
select cardNum from [Transaction] where transType in(
select transTypeID from TransactionType where typeName='Failed'))

--Task5
select userID from [userCard] where cardnum in(
select top 1 cardNum from [transaction] group by cardNum order by count(cardNum) desc)

--Task6
select name from userType where userTypeID in(
select userType from [user] where userid in(
select userId from userCard where cardNum in(
select cardNum from [card] where cardTypeID in(
select cardTypeID from cardType where name='Credit'))))


--Task7
select name,count(*)as TypeCount from CardType group by name  order by count(*) desc

--Task8
select name from [user] where userId in(
select userID from usercard where cardNum in(
select cardNum from [Card] where balance = (select max(balance) from [Card] where balance <> (select max(balance) from [Card]))))
...
select userID from [userCard] where cardnum in(
select cardNum from [Transaction] where transType in(
select transTypeID from TransactionType where typeName='Withdraw'))))